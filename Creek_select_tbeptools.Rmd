...
This workflow was developed as a test case for updating data used to estimate tidal creek assessment cateogires using FLDEP IWR run data.  In this example, we use IWR Run 64, but in theory it should work with any IWR run. The only requirements are:
...
* A WBID polygon shapefile for the appropriate run, which can be retrieved through web services (<https://geodata.dep.state.fl.us/datasets/FDEP::waterbody-ids-wbids/about>.  The Run64 WBID shapefiles are also available from the folder here: <http://publicfiles.dep.state.fl.us/dear/kristin/>. 

Below we set a path to the extracted location for the IWR database, load the tidal creek line file, and load the wbid polygone fil.

```{r}
# set database path
pth <- "~/Desktop/iwr2022_run64_2022-11-29/"
...
Now we import the IWR run data for the entire state using an ODBC connection to the extracted folder.  The following code just iterates through the approac `.accdb` files in the extrated folders, retrieves the data within a ten year window from the current run, and pulls out parameters of interest.  This is done in a loop using a SQL. It only takes a few minutes to run and does not eat up a lot of memory.

```{r}
# this retrieves most recent ten years of iwr data (excluding incomplete current year)
...
Finally, we want to compare the results from the new workflow with those from the original to make sure we've done it right.  We use functions from the tbeptools package to estimate creek scores, then compare in a matrix (this returns the same since tbeptools has already been updatd with IWR run 64).

```{r}
library(tbeptools)

# get new estimates
tmpnew <- anlz_tdlcrk(tidalcreeks, iwrraw, yr = 2022)
